...
class StateProcessor():
    ...
    def __init__(self):
        ...
        with tf.variable_scope("state_processor"):
            self.input_state = tf.placeholder(shape=[210, 160, 3], dtype=tf.uint8,name="original_image")
...
class Estimator():
    ...
    def _build_summary_dir(self, sess):
        if self.summaries_dir:
           summary_dir = os.path.join(self.summaries_dir, "summaries_{}".format(self.scope))
           if not os.path.exists(summary_dir):
               os.makedirs(summary_dir)
...
def deep_q_learning(sess,
    ...
    for i in range(replay_memory_init_size):
        ...
        if done:
            ...
            '''
            print(state)
            print("shape 1: %d", (np.shape(state)))
            sys.stdout.flush()
            '''
            state = np.stack([state] * 4, axis=2)
            '''
            print(state)
            print("shape 2: %d", (np.shape(state)))
            sys.stdout.flush()
            '''
        else:
            state = next_state
            #print("the shap is %d",np.shape(state))
...
with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    q_estimator._build_summary_dir(sess)
